...
JSONPath.prototype.evaluate = function (obj, expr) {
    ...
    var result = this._trace(exprList, obj, ['$'], null, null); // We could add arguments to let user pass parent and its property name in case it needed to access the parent
    ...
    if (result.length === 1 && !this.wrap && !Array.isArray(result[0].value)) {
        ...
        return result[0][this.resultType];
    }
    return result.reduce(function (result, ea) {
        var valOrPath;
        switch (self.resultType) {
        case 'value':
            valOrPath = ea[self.resultType];
            break;
        case 'path':
            valOrPath = self._asPath(ea[self.resultType]);
            break;
        case 'all':
            result.push(ea);
            return result;
...
JSONPath.prototype._trace = function (expr, val, path, parent, parentPropName) {
    ...
    var self = this;
    if (!expr.length) {return {path: path, value: val, parent: parent, parentProperty: parentPropName};}
    ...
    if (val && val.hasOwnProperty(loc)) { // simple case--directly follow property
        addRet(this._trace(x, val[loc], push(path, loc), val, loc));
    }
    else if (loc === '*') { // all child properties
        this._walk(loc, x, val, path, parent, parentPropName, function (m, l, x, v, p, par, pr) {
            addRet(self._trace(unshift(m, x), v, p, par, pr));
        });
    }
    else if (loc === '..') { // all descendent properties
        addRet(this._trace(x, val, path, parent, parentPropName)); // Check remaining expression with val's immediate children
        this._walk(loc, x, val, path, parent, parentPropName, function (m, l, x, v, p) {
            if (typeof v[m] === 'object') { // Keep going with recursive descent on val's object children
                addRet(self._trace(unshift(l, x), v[m], push(p, m), v, m));
            }
        });
    }
        ...
        addRet(this._trace(unshift(this._eval(loc, val, path[path.length - 1], path.slice(0, -1), parent, parentPropName), x), val, path, parent, parentPropName));
    }
    ...
    // ancestor object of val
        ...
        return {path: push(path, loc), value: parentPropName, parent: parent, parentProperty: null};
    }
        ...
        for (parts = loc.split(','), i = 0; i < parts.length; i++) {
            addRet(this._trace(unshift(parts[i], x), val, path, parent, parentPropName));
        }
    }
    else if (/^(-?[0-9]*):(-?[0-9]*):?([0-9]*)$/.test(loc)) { // [start:end:step]  Python slice syntax
        addRet(this._slice(loc, x, val, path, parent, parentPropName));
    }
    ...
    return ret.reduce(function (all, ea) {
        return all.concat(ea.isParentSelector ? self._trace(ea.expr, val, ea.path, parent, parentPropName) : ea);
    }, []);
};

JSONPath.prototype._walk = function (loc, expr, val, path, parent, parentPropName, f) {
    var i, n, m;
    if (Array.isArray(val)) {
        for (i = 0, n = val.length; i < n; i++) {
            f(i, loc, expr, val, path, parent, parentPropName);
        }
    }
    else if (typeof val === 'object') {
        for (m in val) {
            if (val.hasOwnProperty(m)) {
                f(m, loc, expr, val, path, parent, parentPropName);
            }
        }
    }
};

JSONPath.prototype._slice = function (loc, expr, val, path, parent, parentPropName) {
    ...
    for (i = start; i < end; i += step) {
        ret = ret.concat(this._trace(unshift(i, expr), val, path, parent, parentPropName));
    }
    return ret;
};
