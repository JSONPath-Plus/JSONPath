...
function jsonPath(obj, expr, arg) {
   var P = {
      ...
      trace: function(expr, val, path) {
         ...
         if (loc === '^') return path.length ? [{path: push(path, loc), expr: x, parentSelection: true}] : [];
         ...
         return ret.reduce(function(all, ea) {
            return all.concat(ea.parentSelection ? P.trace(ea.expr, val, ea.path) : [ea]);
         }, []);
      },
   ...
   if (expr && obj && (resultType == "value" || resultType == "path")) {
      ...
      if (result.length === 1 && !P.wrap && !Array.isArray(result[0].val)) return result[0][resultType];
