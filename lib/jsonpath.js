...
JSONPath.prototype.evaluate = function (obj, expr) {
    var self = this;
    this._obj = obj;
    if (expr && obj && (this.resultType === 'value' || this.resultType === 'path' || this.resultType === 'pointer')) {
        ...
        return result.reduce(function (result, ea) {
            ...
            if (self.resultType === 'pointer') {valOrPath = self._asPointer(ea['path']);}
...
JSONPath.prototype._asPointer = function (pointer) {
    var i, n, x = pointer, p = '';
    for (i = 1, n = x.length; i < n; i++) {
        p += '/' + x[i].toString()
            .replace(/\~/g, '~0')
