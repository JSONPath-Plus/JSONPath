...
function jsonPath(obj, expr, arg) {
   var P = {
      ...
      trace: function(expr, val, path) {
         ...
         if (!expr || !expr.length) return [{path: path, value: val}];

         var loc = expr[0], x = expr.slice(1);
         // the parent sel computation is handled in the frame above using the
         // ancestor object of val
         if (loc === '^') return [{path: push(path, loc), expr: x, parentSelection: true}];
         ...
         return ret.reduce(function(all, ea) {
            if (!ea.parentSelection) { all.push(ea); return all; }
            return all.concat(P.trace(ea.expr, val, ea.path));
         }, []);
      },
   ...
   if (expr && obj && (resultType == "value" || resultType == "path")) {
      ...
      var result = P.trace(exprList, obj, ["$"]).reduce(function(result, ea) {
