...
JSONPath.prototype.evaluate = function (expr, json, callback) {
    var self = this, flatten = this.flatten, resultType = this.resultType, sandbox = this.sandbox, wrap = this.wrap, preventEval = this.preventEval, callback = callback || this.callback;
    json = json || this.json;
    expr = expr || this.path;
    if (expr && typeof expr === 'object') {
        json = expr.hasOwnProperty('json') ? expr.json : json;
        expr = expr.hasOwnProperty('path') ? expr.path : path;
        flatten = expr.hasOwnProperty('flatten') ? expr.flatten : flatten;
        resultType = expr.hasOwnProperty('resultType') ? expr.resultType : resultType;
        sandbox = expr.hasOwnProperty('sandbox') ? expr.sandbox : sandbox;
        wrap = expr.hasOwnProperty('wrap') ? expr.wrap : wrap;
        preventEval = expr.hasOwnProperty('preventEval') ? expr.preventEval : preventEval;
        callback = expr.hasOwnProperty('callback') ? expr.callback : callback;
    }
    if (Array.isArray(expr)) {
        expr = JSONPath.toPathString(expr);
    }
    if (!expr || !json || allowedResultTypes.indexOf(this.resultType) === -1) {
        return;
    }
    this._obj = json;
    ...
    var result = this._trace(exprList, json, ['$'], null, null, callback); // We could add arguments to let user pass parent and its property name in case it needed to access the parent
...
JSONPath.prototype._eval = function (code, _v, _vname, path, parent, parentPropName) {
    ...
    if (code.indexOf('@path') > -1) {
        this.sandbox._$_path = JSONPath.toPathString(path.concat([_vname]));
